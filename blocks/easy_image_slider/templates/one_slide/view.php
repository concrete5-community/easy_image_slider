...
?>
    ...
    <?php   foreach ($files as $key => $f) :
        ...
        <div class="item" id="item-<?php   echo $key ?>" style="<?php   echo $thumbnailBackground ?>" <?php   if($f->getAttribute('image_bg_color')): ?>data-color="<?php   echo $f->getAttribute('image_bg_color') ?><?php   endif ?>" >
            <?php   if($options->lightbox && !$linkUrl ) : ?><a href="<?php   echo $fullUrl ?>" data-image="<?php   echo $fullUrl ?>" <?php   if($options->lightboxTitle) : ?> title="<b><?php   echo $f->getTitle() ?></b><?php   if($options->lightboxDescription) : ?><br /><?php   echo $f->getDescription() ?><?php   endif ?>"<?php   endif ?> ><?php   endif ?>       
